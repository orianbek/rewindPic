    ...
    public BufferedImage changeColor() {
        ...
        for (int x = 0; x < editedImage.getWidth(); x++) {
            for (int y = 0; y < editedImage.getHeight(); y++) {
                ...
                if (c.getRed() < MIDDLE_COLOR) {
                    red = c.getRed() + MIDDLE_COLOR;
                } else {
                    red = c.getRed() - MIDDLE_COLOR;
                }
                if (c.getGreen() < MIDDLE_COLOR) {
                    green = c.getGreen() + MIDDLE_COLOR;
                } else {
                    green = c.getGreen() - MIDDLE_COLOR;
                }
                if (c.getBlue() < MIDDLE_COLOR) {
                    blue = c.getBlue() + MIDDLE_COLOR;
                } else {
                    blue = c.getBlue() - MIDDLE_COLOR;
                }
...
// Pack RGB values into a single integer
    ...
    public void togglePaintPicture() {
